...
def load_model_and_tokenizer(
    ...
    Union[BertForPreTraining, RobertaForMaskedLM, AlbertForPreTraining],
    ...
    Returns:
        model (Union[BertForPreTraining, RobertaForMaskedLM, AlbertForPreTraining]): A pre-trained or checkpoint model.
        tokenizer (Union[BertTokenizer, RobertaTokenizer, AlbertTokenizer]): A pre-trained tokenizer.
    """
    if args.use_ckpt:
        # load from checkpoint
        if "bert" in version:
            model = BertForPreTraining.from_pretrained(args.ckpt_dir)
        elif "roberta" in version:
            model = RobertaForMaskedLM.from_pretrained(args.ckpt_dir)
        else:
            model = AlbertForPreTraining.from_pretrained(args.ckpt_dir)
    else:
        # load from huggingface
        if "bert" in version:
            model = BertForPreTraining.from_pretrained(version)
        elif "roberta" in version:
...
def get_encodings(
    data_keys: List[str],
    data: Dict[str, str],
    model: Union[BertForPreTraining, RobertaForMaskedLM, AlbertForPreTraining],
    ...
    Args:
        ...
        model (Union[BertForPreTraining, RobertaForMaskedLM, AlbertForPreTraining]): A Pre-trained PreTrainedModel model.
    ...
    for i, key in enumerate(data_keys):
        ...
        for sent in data[key]["examples"]:
            ...
            # get BERT outputs
            if type(model) == BertForPreTraining:
