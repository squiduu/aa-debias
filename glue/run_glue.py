...
from glue_utils import clear_console, get_glue_logger, DebiasedEncoder
...
def run_glue(
    ...
    if data_args.task_name is not None:
        ...
        raw_datasets = load_dataset(
            path="glue", name=data_args.task_name, cache_dir=model_args.cache_dir
        )
    ...
    if training_args.resume_from_checkpoint:
        model = DebiasedEncoder(
            model_name_or_path=model_args.model_name_or_path,
            config=config,
            ckpt_path=training_args.resume_from_checkpoint,
        )
    else:
    ...
    def preprocess_glue(examples: Dict[str, List]) -> BatchEncoding:
        ...
        result = tokenizer.__call__(
            *inputs, padding=padding, max_length=max_seq_length, truncation=True
        )

        # map labels to ids (not necessary for GLUE tasks)
        if label2id is not None and "label" in examples:
            result["label"] = [
                (label2id[label] if label != -1 else -1) for label in examples["label"]
            ]
    ...
    if training_args.do_train:
        ...
        if data_args.max_train_samples is not None:
            train_dataset = train_dataset.select(
                indices=range(data_args.max_train_samples)
            )
    ...
    if training_args.do_eval:
        ...
        if data_args.max_eval_samples is not None:
            eval_dataset = eval_dataset.select(
                indices=range(data_args.max_eval_samples)
            )
    ...
    def compute_metrics(eval_pred: EvalPrediction) -> Dict[str, np.ndarray]:
        ...
        if data_args.task_name is not None:
            ...
            return {
                "accuracy": (preds == eval_pred.label_ids)
                .astype(np.float32)
                .mean()
                .item()
            }
