...
def auto_debias(args: Namespace, logger: Logger):
    ...
    model, tokenizer = prepare_model_and_tokenizer(
        model_name=args.model_name, model_version=args.model_version, run_type="auto-debias"
    )
    ...
    # clean target words
    logger.info(f"Remove words that contains OOV tokens.")
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        if epoch == args.max_epochs:
            ...
            model_for_glue, model_for_seat = overwrite_state_dict(trained_model=model, args=args)
