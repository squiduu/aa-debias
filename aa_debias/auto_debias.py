...
def auto_debias(args: Namespace, logger: Logger):
    ...
    for i in range(1, args.max_epochs + 1):
        ...
        for iter, batch_idx in enumerate(dataloader):
            ...
            if (iter + 1) % 500 == 0:
                logger.info(f"Epoch: {i}, Iter: {iter + 1}/{len(dataloader)}, Batch Loss: {batch_loss:.4f}")
        ...
        # after an epoch
        logger.info(f"Epoch: {i}, Epoch Loss: {epoch_loss / len(dataloader):.4f}")
        # save debiased model and tokenizer every an epoch
        logger.info(
            f"Save debiased model and tokenizer: {args.save_dir + f'{args.model_version}_{args.debias_type}_epoch:{i}'}"
        )
        model.save_pretrained(args.save_dir + f"{args.model_version}_{args.debias_type}_epoch:{i}")
        tokenizer.save_pretrained(args.save_dir + f"{args.model_version}_{args.debias_type}_epoch:{i}")
