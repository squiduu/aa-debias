...
def aa_debias(args: Namespace, logger: Logger):
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        if epoch == 10:
            ...
            logger.info(
                f"Save debiased model and tokenizer: {args.output_dir + f'{args.model_name}_{args.debias_type}_epoch:{epoch}'}"
            )
            tuning_model.save_pretrained(args.output_dir + f"{args.model_name}_{args.debias_type}_epoch:{epoch}")
            tokenizer.save_pretrained(args.output_dir + f"{args.model_name}_{args.debias_type}_epoch:{epoch}")
