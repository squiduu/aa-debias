...
from aa_utils import (
    ...
    JSDivergence,
    get_logits,
    get_abs_cosine_similarity,
    set_seed,
)


def run_aa_debias(args: Namespace, logger: Logger):
    ...
    freezing_model, tuning_model, tokenizer = prepare_model_and_tokenizer(
        model_name=args.model_name, model_version=args.model_version, run_type=args.mode
    )
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        for iter, batch_idx in enumerate(dataloader):
            ...
            freezing_jsd = jsd_module.forward(net1_logits=targ1_freezing_logits, net2_logits=targ2_freezing_logits)
            tuning_jsd = jsd_module.forward(net1_logits=targ1_tuning_logits, net2_logits=targ2_tuning_logits)
            distance = torch.abs(freezing_jsd - tuning_jsd)
            ...
            tuning_cos_sim = get_abs_cosine_similarity(logits1=targ1_tuning_logits, logits2=targ2_tuning_logits)
            inverse_cos_sim = 1 - tuning_cos_sim

            # get loss for a batch
            batch_loss = distance + inverse_cos_sim
            ...
            if (iter + 1) % 100 == 0:
                logger.info(
                    f"Epoch: {epoch} - Iter: {iter + 1}/{len(dataloader)} - Batch Loss: {batch_loss:.4f} - Distance: {distance:.4f} - Inverse Cos Sim: {inverse_cos_sim:.4f}"
                )
        ...
        if epoch == args.max_epochs:
            ...
            model_for_glue, model_for_seat = overwrite_state_dict(trained_model=tuning_model, args=args)
            # save sequence classification model for GLUE
            model_for_glue.save_pretrained(
                args.output_dir + f"{args.model_name}4glue_{args.run_name}_{args.debias_type}_epoch:{epoch}"
            )
            tokenizer.save_pretrained(
                args.output_dir + f"{args.model_name}4glue_{args.run_name}_{args.debias_type}_epoch:{epoch}"
            )
            # save encoder model model for GLUE
            model_for_seat.save_pretrained(
                args.output_dir + f"{args.model_name}4seat_{args.run_name}_{args.debias_type}_epoch:{epoch}"
            )
            tokenizer.save_pretrained(
                args.output_dir + f"{args.model_name}4seat_{args.run_name}_{args.debias_type}_epoch:{epoch}"
            )
...
if __name__ == "__main__":
    ...
    run_aa_debias(args=args, logger=logger)
