...
def generate_prompts(args: Namespace, logger: Logger):
    ...
    logger.info(f"Load and tokenize stereotype words: {args.data_dir + 'stereotype_words.txt'}")
    ...
    # load prompt words
    logger.info(f"Load and tokenize prompt words: {args.data_dir + 'wiki_words_5000.txt'}")
    ...
    logger.info(f"Create and open prompt file: {args.data_dir + f'prompts_{args.model_version}_{args.debias_type}'}")
    prompt_file = open(
    ...
    logger.info(f"Get JS-Divergence values for all prompts about {args.debias_type} bias.")
    for i in range(args.max_prompt_len):
        ...
        if args.debias_type == "gender":
            current_prompts_js_div_values = get_prompt_jsd(
        ...
        selected_prompts = np.array(current_prompts)[np.argsort(current_prompts_js_div_values)[::-1][: args.top_k]]
