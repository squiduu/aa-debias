...
def prepare_model_and_tokenizer(
    model_name: str, model_version: str, run_type: str
) -> Union[
    Tuple[
    ...
    if run_type in ["auto-debias", "generate"]:
...
def get_abs_cosine_similarity(logits1: torch.FloatTensor, logits2: torch.FloatTensor) -> torch.FloatTensor:
...
def get_prompt_jsd(
    ...
    for i in tqdm(range(len(targ1_words))):
        ...
        prompt_jsd_values.append(js_div_values)
...
def overwrite_state_dict(
    ...
    if type(trained_model) == BertForMaskedLM:
        model_class_for_glue = BertForSequenceClassification
        model_class_for_seat = BertModel
    elif type(trained_model) == RobertaForMaskedLM:
        model_class_for_glue = RobertaForSequenceClassification
        model_class_for_seat = RobertaModel
    else:
        model_class_for_glue = AlbertForSequenceClassification
        model_class_for_seat = AlbertModel

    # get initialized pre-trained model
    model_for_glue = model_class_for_glue.from_pretrained(args.model_version)
    model_for_seat = model_class_for_seat.from_pretrained(args.model_version)

    # get initialized pre-trained model weights
    model_dict_for_glue = model_for_glue.state_dict()
    model_dict_for_seat = model_for_seat.state_dict()
    # filter out unnecessary keys in debiased masked model
    trained_model_dict_for_glue = {k: v for k, v in trained_model.state_dict().items() if k in model_dict_for_glue}
    if model_class_for_seat == BertModel:
        trained_model_dict_for_seat = {
            k[5:]: v for k, v in trained_model.state_dict().items() if k[5:] in model_dict_for_seat
        }
    elif model_class_for_seat == RobertaModel:
        trained_model_dict_for_seat = {
            k[8:]: v for k, v in trained_model.state_dict().items() if k[8:] in model_dict_for_seat
        }
    else:
        trained_model_dict_for_seat = {
            k[7:]: v for k, v in trained_model.state_dict().items() if k[7:] in model_dict_for_seat
        }

    # overwrite entries in the existing initialized state dict
    model_dict_for_glue.update(trained_model_dict_for_glue)
    model_dict_for_seat.update(trained_model_dict_for_seat)

    # overwrite updated weights
    model_for_glue.load_state_dict(model_dict_for_glue)
    model_for_seat.load_state_dict(model_dict_for_seat)

    return model_for_glue, model_for_seat
