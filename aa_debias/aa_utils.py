...
def get_aa_logger(args: Namespace) -> Logger:
    ...
    if "generate" in sys.argv[0]:
        file_hdlr = logging.FileHandler(filename=args.output_dir + f"prompt_{args.run_no}.log")
    elif "auto" in sys.argv[0]:
        file_hdlr = logging.FileHandler(filename=args.output_dir + f"auto_{args.run_no}.log")
    elif "aa" in sys.argv[0]:
        file_hdlr = logging.FileHandler(filename=args.output_dir + f"aa_{args.run_no}.log")
    file_hdlr.setFormatter(fmtr)
    logger.addHandler(file_hdlr)

    # notify to start
    logger.info(f"Run number: {args.run_no}")
...
) -> Union[Tuple[PreTrainedModel, PreTrainedTokenizer], Tuple[PreTrainedModel, PreTrainedModel, PreTrainedTokenizer]]:
    """Download and prepare the pre-trained model and tokenizer.

    Args:
        model_name (str): A name of pre-trained model e.g., bert, gpt2
        model_version (str): A version of pre-trained model e.g., bert-base-uncased, gpt2-large
        mode (str): A status of either generating prompts or debiasing models.

    Raises:
        NotImplementedError: An error that occurs when the input `model_name` is not a candidate.

    Returns:
        Union[Tuple[PreTrainedModel, PreTrainedTokenizer], Tuple[PreTrainedModel, PreTrainedModel, PreTrainedTokenizer]]: A tuple of pre-trained model and tokenizer.
    """
    if mode == "auto-debias":
        if model_name == "bert":
            tokenizer = BertTokenizer.from_pretrained(model_version)
            model = BertForPreTraining.from_pretrained(model_version)

        elif model_name == "roberta":
            tokenizer = RobertaTokenizer.from_pretrained(model_version)
            model = RobertaForMaskedLM.from_pretrained(model_version)
            model.roberta = RobertaModel.from_pretrained(model_version)
        ...
        if model_name == "bert":
            tokenizer = BertTokenizer.from_pretrained(model_version)
            freezing_model = BertForPreTraining.from_pretrained(model_version)
            tuning_model = BertForPreTraining.from_pretrained(model_version)
            ...
            freezing_model.roberta = RobertaModel.from_pretrained(model_version)
            tuning_model.roberta = RobertaModel.from_pretrained(model_version)

        elif model_name == "albert":
            tokenizer = AlbertTokenizer.from_pretrained(model_version)
            freezing_model = AlbertForPreTraining.from_pretrained(model_version)
            tuning_model = AlbertForPreTraining.from_pretrained(model_version)
        ...
        if model_name == "bert":
            ...
            tokenizer = AlbertTokenizer.from_pretrained(model_version)
            model = AlbertForPreTraining.from_pretrained(model_version)
...
def clear_words(
    _words1: List[str], _words2: List[str], tokenizer: PreTrainedTokenizer, mode: str
    ...
    Args:
        ...
        tokenizer (PreTrainedTokenizer): A pre-trained tokenizer.
        mode (str): A status of either generating prompts or debiasing models.
...
def tokenize_ith_prompts(
    prompts: List[str], targ1_word: str, targ2_word: str, tokenizer: PreTrainedTokenizer
...
def tokenize_prompts(
    prompts: List[str], targ1_words: List[str], targ2_words: List[str], tokenizer: PreTrainedTokenizer
) -> Tuple[BatchEncoding, BatchEncoding, np.ndarray, np.ndarray]:
...
def get_batch_inputs(
    ...
    try:
        for key in targ1_tokens.keys():
            # slice to batch size
            targ1_inputs[key] = targ1_tokens[key][args.batch_size * idx : args.batch_size * (idx + 1)]
            targ2_inputs[key] = targ2_tokens[key][args.batch_size * idx : args.batch_size * (idx + 1)]

        targ1_local_mask_idx = targ1_mask_idx[args.batch_size * idx : args.batch_size * (idx + 1)]
        targ2_local_mask_idx = targ2_mask_idx[args.batch_size * idx : args.batch_size * (idx + 1)]

    except IndexError:
        for key in targ1_tokens.keys():
            # get rest of batches
            targ1_inputs[key] = targ1_tokens[key][args.batch_size * (idx + 1) :]
            targ2_inputs[key] = targ2_tokens[key][args.batch_size * (idx + 1) :]

        targ1_local_mask_idx = targ1_mask_idx[args.batch_size * (idx + 1) :]
        targ2_local_mask_idx = targ2_mask_idx[args.batch_size * (idx + 1) :]

    return targ1_inputs, targ2_inputs, targ1_local_mask_idx, targ2_local_mask_idx


def get_logits(
    freezing_model: PreTrainedModel,
    tuning_model: PreTrainedModel,
    ...
    Args:
        freezing_model (PreTrainedModel): A pre-trained language model for freezing.
        tuning_model (PreTrainedModel): A pre-trained language model for fine-tuning.
    ...
    if mode == "auto-debias" and freezing_model is None and tuning_model is not None:
        ...
        if finetune_ids is not None:
            _logits = outputs.prediction_logits[torch.arange(torch.Tensor.size(outputs.prediction_logits)[0]), mask_idx]
            logits = _logits[:, finetune_ids]
        else:
            logits = outputs.prediction_logits[torch.arange(torch.Tensor.size(outputs.prediction_logits)[0]), mask_idx]
        ...
        freezing_logits = freezing_outputs.prediction_logits[
            torch.arange(torch.Tensor.size(freezing_outputs.prediction_logits)[0]), mask_idx
        ]
        tuning_logits = tuning_outputs.prediction_logits[
            torch.arange(torch.Tensor.size(tuning_outputs.prediction_logits)[0]), mask_idx
        ]
...
def get_cosine_similarity(logits1: torch.FloatTensor, logits2: torch.FloatTensor) -> torch.FloatTensor:
    cos_sim = F.cosine_similarity(x1=logits1, x2=logits2)
...
def get_js_div_values(
    ...
    for i in range(torch.Tensor.size(targ1_tokens["input_ids"])[0] // args.batch_size + 1):
...
def get_prompt_js_div(
    ...
    Returns:
        np.mean(prompt_js_divs, axis=0) (np.ndarray): Accumulated JS-Divergence values for all prompts.
    """
    ...
    return np.mean(prompt_js_div_values, axis=0)
